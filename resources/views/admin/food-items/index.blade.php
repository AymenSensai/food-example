                ...
                <div class="p-6">
                    <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
                        <div class="relative w-full md:w-auto">
                            <form action="{{ route('admin.food-items.index') }}" method="GET" x-data>
                        ...
                        <a href="{{ route('admin.food-items.create') }}" class="inline-flex justify-center w-full md:w-auto items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-lg shadow-lg shadow-orange-500/30 transition-all duration-200 gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <table class="min-w-full divide-y divide-slate-200">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider hidden md:table-cell">Catégorie</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider hidden md:table-cell">Prix</th>
                                ...
                                @foreach ($foodItems as $item)
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($item->image)
                                                <img src="{{ Str::startsWith($item->image, 'http') ? $item->image : Storage::temporaryUrl($item->image, now()->addMinutes(60)) }}" alt="" class="h-10 w-10 rounded-full object-cover border border-slate-200">
                                            @else
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                            <div class="text-sm font-bold text-orange-600">{{ $item->price }} DA</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.food-items.edit', $item) }}" class="text-slate-600 hover:text-orange-600 mr-3 transition-colors">
                                                <span class="sr-only">Modifier</span>
                                                <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                </svg>
                                            </a>
                                            <form action="{{ route('admin.food-items.destroy', $item) }}" method="POST" class="inline-block" onsubmit="return confirm('Êtes-vous sûr ?');">
                                                @csrf
                                                @method('DELETE')
